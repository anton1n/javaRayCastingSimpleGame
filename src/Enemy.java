...
public abstract class Enemy extends Entity {
    ...
    public Enemy(double x, double y, double range, double speed, String spriteSheetPath, Map<State, Integer> frameCounts, int frameWidth, int frameHeight, int frameDuration, String name) {
        this.range = range;
        this.speed = speed;
    ...
    protected double updateMovement(PositionComponent playerPosition, PositionComponent enemyPosition) {
        double dx = playerPosition.x - enemyPosition.x;
        double dy = playerPosition.y - enemyPosition.y;
        double distance = Math.sqrt(dx * dx + dy * dy);
        ...
        if (distance > 0 && distance < range && state != State.attacking) {
            ...
            if (Camera.isWalkable((int)(enemyPosition.x + moveX), (int)(enemyPosition.y + moveY))) {
    ...
    protected void moveToward(double px, double py) {
    ...
    protected void onDeath(HealthComponent healthComponent) {
        System.out.println(name +  " died");
    }

    void setHealth(int health){
        getComponent(HealthComponent.class).setHealth(health);
    }
